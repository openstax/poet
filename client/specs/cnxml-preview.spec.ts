...
describe('cnxml-preview', () => {
  ...
  describe('complex', () => {
    ...
    function setActiveEditor(uri: Uri) {
    ...
    beforeEach(() => {
      ...
      fs[join(fakeWorkspacePath, 'modules')] = mockfs.load(join(actualWorkspaceRootDir, 'modules'))
      mockfs(fs)
    })
    afterEach(() => {
      mockfs.restore()
    ...
    it('cnxml preview rebinds to resource in the active editor', async () => {
      ...
      const resourceFirst = uri.with({ path: join(uri.path, 'modules', 'm00001', 'index.cnxml') })
      const resourceSecond = uri.with({ path: join(uri.path, 'modules', 'm00002', 'index.cnxml') })
