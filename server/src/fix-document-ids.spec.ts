...
import { fixDocument, fixModule, padLeft } from './fix-document-ids'
import mockfs from 'mock-fs'
import { pageMaker } from './model/page.spec'
import { Bundle } from './model/bundle'
import { FS_PATH_HELPER, loadSuccess } from './model/util.spec'
...
describe('Element ID creation', () => {
  ...
  describe('fixModule', () => {
    ...
    it('loads the file from the filesystem and saves it back', async () => {
      const bundle = new Bundle(FS_PATH_HELPER, process.cwd())
      const rootUri = FS_PATH_HELPER.join(FS_PATH_HELPER.dirname(bundle.absPath), '..')
      ...
      // Verify that an id attribute was added to the page (the <para>)
      loadSuccess(page, true)
      expect(page.elementIds.toArray()).not.toEqual([])
    })
  })
})
