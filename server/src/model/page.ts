...
export class PageValidationKind extends ValidationKind {
  ...
  static EXERCISE_COUNT_ZERO = new PageValidationKind('Expected 1 exercise with this tag but found 0', ValidationSeverity.INFORMATION)
  static EXERCISE_COUNT_TOO_MANY = new PageValidationKind('Expected 1 exercise with this tag but found too many', ValidationSeverity.INFORMATION)
  static EXERCISE_NO_PAGES = new PageValidationKind('Did not find any pages in our bundle for the context for this exercise', ValidationSeverity.INFORMATION)
  static EXERCISE_PAGE_MISSING_FEATURE = new PageValidationKind('Exercise contains a context element ID but that ID is not available on the current Page because no target Page was specified in the exercise', ValidationSeverity.INFORMATION)
  static EXERCISE_MISSING_TARGET_FEATURE = new PageValidationKind('Exercise contains a context element ID but that ID is not available on the context Page. If the element ID exists in this file then this error could be because a cnxmod tag is in this book but does not have that feature id. If so, remove the cnxmod tag', ValidationSeverity.INFORMATION)
}
