...
const ALLOWED_FILES = ["LICENSE", "README.md", ".gitpod.yml"];

// Print log messages to stderr per convention
const log = console.error.bind(console.error);

log(
  "WARN: Manually setting NODE_ENV=production so we get nicer error messages"
);
...
function loadNode(n: Fileish) {
  const exists = fs.existsSync(n.absPath);
  if (!exists) {
    throw new Error(`Could not find file ${n.absPath}`);
  }
  const bits = exists ? fs.readFileSync(n.absPath, "utf-8") : undefined;
  n.load(bits);
}
...
function loadRepo(repoPath: string) {
  ...
  do {
    nodesToLoad = bundle.allNodes
      .flatMap((n) => n.validationErrors.nodesToLoad)
      .filter((n) => !n.isLoaded && n.validationErrors.errors.size === 0);
...
async function load(bookDirs: string[]): Promise<[boolean, Bundle[]]> {
  ...
  for (const rootPath of bookDirs) {
    ...
    const validationErrors = bundle.allNodes.flatMap(
      (n) => n.validationErrors.errors
    );
    bundles.push(bundle);
...
async function lintLinks(bookDirs: string[]) {
  ...
  for (const bundle of bundles) {
    for (const page of bundle.allPages.all.filter(
      (page) => page.isLoaded && page.exists
    )) {
      for (const link of page.pageLinks) {
        if (link.type === PageLinkKind.URL) {
          ...
          try {
            ...
            hasErrors = true;
            log(
              `Error: Could not parse URL '${url}' and urlEncoded to show any odd unicode characters`,
              encodeURI(url)
            );
            continue;
          }
...
async function orphans(bookDirs: string[]) {
  ...
  for (const bundle of bundles) {
    ...
    const allFiles = I.Set(
      glob.sync("**/*", {
        cwd: repoRoot,
        absolute: true,
        nodir: true,
        ignore: ALLOWED_FILES,
      })
    );
...
function traverse(
  node: BookNode | TocNodeWithRange,
  indexes: number[]
): TocPageWithRange[] {
  if (node instanceof BookNode) {
    ...
    if (indexes.length !== 0) {
      throw new Error(
        `Encountered a Page earlier than expected. The ToC indexes indicate we should keep going but the ToC tree has already arrived at this page: '${node.page.absPath}'`
      );
    }
    return [node];
  } else {
...
function trimNodes(
  node: ClientTocNode | BookToc,
  keepPages: Set<PageNode>
): boolean {
  if (node.type === TocNodeKind.Page) {
    const hasKeeper =
      [...keepPages].find((n) => n.absPath === node.value.absPath) !==
      undefined;
    return !hasKeeper;
  } else {
...
function pageAccumulator(acc: Set<PageNode>, page: PageNode) {
  if (!acc.has(page)) {
    ...
    page.pageLinks.forEach((l) => {
      if (
        l.type === PageLinkKind.PAGE ||
        l.type === PageLinkKind.PAGE_ELEMENT
      ) {
...
async function shrink(repoDir: string, entries: MinDefinition[]) {
  ...
  entries.forEach((entry) => {
    const book = expectValue(
      bundle.books.find((b) => b.slug === entry.slug),
      `Could not find book with slug '${entry.slug}'`
    );
      ...
      if (
        !keepSlugs.has(
          expectValue(
            ...
            "BUG: slug attribute is missing on book element"
          )
        )
      ) {
        expectValue(
          el.parentNode,
          "BUG: the book element clearly has a parent element"
        ).removeChild(el);
      }
    });
...
(async function () {
  switch (process.argv[2]) {
    case "validate": {
      const bookDirs =
        process.argv.length >= 4 ? process.argv.slice(3) : [process.cwd()];
    ...
    case "shrink": {
      ...
      const entries = args.map((entry) => {
        ...
        const sections = sections1.map((s) =>
          s.split(".").map((nStr) => Number.parseInt(nStr))
        );
    ...
    default: {
      log(
        `Unsupported command '${process.argv[2]}'. Expected one of the following:`
      );
